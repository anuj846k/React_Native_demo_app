      ...
      >
        ...
        <View className="flex flex-row justify-center mt-5">
          <View className="flex flex-col items-center relative mt-5">
            ...
            />
            <TouchableOpacity className="absolute bottom-10 right-2 bg-white rounded-full p-1 ">
        ...
        <View className="flex flex-col mt-5">
          <SettingItems title="My bookings" icon={icons.calendar} />
          <SettingItems title="Payments" icon={icons.wallet} />
        </View>
        <View className="flex flex-col mt-5 border-t pt-5 border-primary-200">
          {settings.slice(2).map((item, index) => (
            <SettingItems icon={item.icon} title={item.title} key={index} />
          ))}
        </View>
